...
const SignUp: React.FC = () => {
    ...
    const LoginSchema = yup.object({
        ...
        telephone: yup.string().required("Telephone is required").matches(/^\+250\d{9}$/),
        names: yup.string().required("Names are required").matches(/^[a-zA-Z\s]+$/, "Names must only contain letters and spaces"),
    ...
    const {
        ...
        resolver: yupResolver(LoginSchema) as Resolver<SignUpInputs, any>,
        mode: "onTouched"
    })


    const onSubmit: SubmitHandler<SignUpInputs> = async (data) => {
        await signUp({ setLoading, email: data.email, password: data.password, telephone: data.telephone, names: data.names })
    }
    ...
    return (
        <div className='w-full flex flex-col items-center pt-[20vh] min-h-screen'>
            ...
            <div className='flex flex-col rounded-xl border border-slate-200 p-10 w-11/12 msm:w-[80%] sm:w-[70%] md:w-[50%] mlg:w-[40%] lg:w-[35%] 2xl:w-[28%] bg-white items-center'>
                <span className='text-2xl my-1'> RCA Template</span>
                <span className='font-light my-1'>Login to start using RCA Template </span>
                <form onSubmit={handleSubmit(onSubmit)} className='my-3 w-full flex flex-col items-center'>
                <div className='w-full my-2'>
                        ...
                        />
                        {errors.email && <span className='text-red-400 text-[16px]'>{errors?.names?.message}</span>}
                    </div>
                    <div className='w-full my-2'>
                        <label className="text-sm font-medium leading-none text-gray-800">
                            Email
                    ...
                    <div className='w-full my-2'>
                        ...
                        />
                        {errors.email && <span className='text-red-400 text-[16px]'>{errors?.telephone?.message}</span>}
                    </div>
                    <div className="w-full my-2">
                        <label className="text-sm font-medium leading-none text-gray-800">
                            Password
                        ...
                        <div className="relative flex items-center justify-center">
                            <input
                                ...
                                {...register("password", { required: true, pattern: /^(?=.*[A-Z])(?=.*\d)(?=.*[\W_])[A-Za-z\d\W_]{6,}$/, })}
                            />
                ...
                <p className={`text-[12px] text-[#212121]`}>If you have an account <span className={`text-blue-500 cursor-pointer`} onClick={() => window.location.replace("/auth/login")}>click here to sign in</span></p>
            </div>
        </div>
    )
}
